...
let totalCookieMined = 0;
let totalCookieSpent = 0;

const totalCookieMinedElement = document.getElementById('total-cookies-mined')
const totalCookieSpentElement = document.getElementById('total-cookies-spent')

let minerUpgradeCost = 25;
let minerSpeed = 1;

const minerUpgradeCostElement = document.getElementById('miner-upgrade-cost')
const minerSpeedElement = document.getElementById('miner-speed')
...
function render() {
  ...
  totalCookieMinedElement.innerHTML = totalCookieMined;
  totalCookieSpentElement.innerHTML = totalCookieSpent;
  minerUpgradeCostElement.innerHTML = minerUpgradeCost;
  minerSpeedElement.innerHTML = minerSpeed;
}
...
function getCookie() {
  ...
  totalCookieMined += cookiesPerClick;
  render()
}
...
function mineCookies() {
  ...
  totalCookieMined += (minersCount * cookiesPerClick);
  render()
}

function upgradeMiner() {
  if (cookieCount >= minerUpgradeCost) {
    cookieCount -= minerUpgradeCost;
    totalCookieSpent += minerUpgradeCost;
    minerSpeed += 1;
...
function saveAll() {
  ...
  if (totalCookieMined > 0) {localStorage.setItem('totalCookieMined', totalCookieMined.toString ())};
  if (totalCookieSpent > 0) {localStorage.setItem('totalCookieSpent', totalCookieSpent.toString ())};
  ...
  if (minerSpeed > 1) {localStorage.setItem('minerSpeed', minerSpeed.toString())};
  if (minerUpgradeCost != 25) {localStorage.setItem('minerUpgradeCost', minerUpgradeCost.toString())};
}

function loadAll() {
 cookieCount = localStorage.getItem('cookieCount');
 totalCookieMined = localStorage.getItem('totalCookieMined');
 totalCookieSpent = localStorage.getItem('totalCookieSpent');
 minerCost = localStorage.getItem('minerCost');
 minersCount = localStorage.getItem('minersCount');
 minerSpeed = localStorage.getItem('minerSpeed');
 minerUpgradeCost = localStorage.getItem('minerUpgradeCost');
}
